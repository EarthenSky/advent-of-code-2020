...
//              There will never be 3 newlines in a row (10s)

// ****************************************************************************************************************** //

> // store a zero at position zero so that the backtracking knows when to end
...
[ // Loop will exit on EOF
    ...
    [ // enters loop if pivot is NOT 10
        ...
        > + > // set tmp to be 1 (loop end) & move to start of spill
        ...
        // Jump to corresponding space in dat section and reduce it to zero then add 1
        >>>> >>>> >>>> >>>> >>>> >>>> >>> [ - ] + <<<< <<<< <<<< <<<< <<<< <<<< <<<
    ...
    [
        ...
        [ //case: is a 10
            ...
            // Assumption: the byte directly after dat is always zero
            -- [ ++ > [ - < + > ] < < -- ] ++ // carry sum until reaching end of dat section
            -- // remove end of dat section mark

            // copy number of answers that at least one person in a group answered to the pivot
            > [ - < <<<< <<<< <<<< <<<< <<<< <<<< <<< < + > >>>> >>>> >>>> >>>> >>>> >>>> >>> > ] <
            <<<< <<<< <<<< <<<< <<<< <<<< <<<< // goto pivot
...
// It's too hard for me to convert the 2 cell number into 6 cells of base 10 numbers & I'm too lazy so just
// get the values from the memory & multiply cell 0 by 256 then add cell 1
